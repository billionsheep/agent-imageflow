...
import {
  ...
  type AgentImageflowReferenceDiagnostics,
...
import { getProjectReferenceDiagnosticsCard, getProjectVisualContextReadiness } from '../lib/projectContextPanel'
...
export default function ProjectContextModal() {
  ...
  const [referenceDiagnostics, setReferenceDiagnostics] = useState<AgentImageflowReferenceDiagnostics | null>(null)
  ...
  const diagnosticsCard = useMemo(() => getProjectReferenceDiagnosticsCard(referenceDiagnostics), [referenceDiagnostics])
  ...
  const loadContext = useCallback(async () => {
    ...
    try {
      ...
      setReferenceDiagnostics(response.reference_diagnostics ?? null)
      setUnauthorized(false)
    } catch (nextError) {
  ...
  const saveContext = useCallback(async (nextContext: AgentImageflowProjectVisualContext, successMessage: string) => {
    ...
    try {
      ...
      setReferenceDiagnostics(reloaded.reference_diagnostics ?? null)
      setUnauthorized(false)
      ...
      >
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-gray-200/70 px-4 py-3 dark:border-white/[0.08]">
          <div className="min-w-0">
            ...
            {diagnosticsCard && (
              <section className="mt-3 rounded-2xl border border-sky-200/80 bg-sky-50/70 p-3 dark:border-sky-500/20 dark:bg-sky-500/10">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="text-xs font-semibold text-sky-900 dark:text-sky-100">参考诊断</div>
                    <div className="mt-1 text-[11px] leading-relaxed text-sky-800/90 dark:text-sky-100/90">
                      {diagnosticsCard.summary}
                    </div>
                  </div>
                  <div className="rounded-full border border-sky-200 bg-white/80 px-2.5 py-1 text-[11px] font-medium text-sky-700 dark:border-sky-400/20 dark:bg-white/[0.06] dark:text-sky-200">
                    主判断：{diagnosticsCard.readiness}
                  </div>
                </div>
                <div className="mt-3 flex flex-wrap gap-2">
                  {diagnosticsCard.labels.length > 0 ? diagnosticsCard.labels.map((label) => (
                    <span key={label} className="rounded-full border border-sky-200 bg-white/75 px-2.5 py-1 text-[11px] text-sky-700 dark:border-sky-400/20 dark:bg-white/[0.06] dark:text-sky-200">
                      {label}
                    </span>
                  )) : (
                    <span className="rounded-full border border-sky-200 bg-white/75 px-2.5 py-1 text-[11px] text-sky-700 dark:border-sky-400/20 dark:bg-white/[0.06] dark:text-sky-200">
                      暂无诊断标签
                    </span>
                  )}
                </div>
                <div className="mt-3 grid gap-2 lg:grid-cols-2">
                  <div className="grid gap-2 sm:grid-cols-3">
                    {diagnosticsCard.counts.map((item) => (
                      <div key={item.label} className="rounded-xl border border-sky-200/70 bg-white/70 px-3 py-2 text-xs dark:border-sky-400/20 dark:bg-white/[0.04]">
                        <div className="text-[10px] font-semibold uppercase tracking-wide text-sky-500/80 dark:text-sky-300/70">{item.label}</div>
                        <div className="mt-1 text-sm font-semibold text-sky-900 dark:text-sky-100">{item.value}</div>
                      </div>
                    ))}
                  </div>
                  <div className="grid gap-2 sm:grid-cols-3">
                    {diagnosticsCard.checks.map((item) => (
                      <div key={item.label} className="rounded-xl border border-sky-200/70 bg-white/70 px-3 py-2 text-xs dark:border-sky-400/20 dark:bg-white/[0.04]">
                        <div className="text-[10px] font-semibold uppercase tracking-wide text-sky-500/80 dark:text-sky-300/70">{item.label}</div>
                        <div className="mt-1 text-sm font-semibold text-sky-900 dark:text-sky-100">{item.value}</div>
                      </div>
                    ))}
                    <div className="rounded-xl border border-sky-200/70 bg-white/70 px-3 py-2 text-xs dark:border-sky-400/20 dark:bg-white/[0.04]">
                      <div className="text-[10px] font-semibold uppercase tracking-wide text-sky-500/80 dark:text-sky-300/70">Provider 参考风险</div>
                      <div className="mt-1 text-sm font-semibold text-sky-900 dark:text-sky-100">{diagnosticsCard.providerRisk}</div>
                    </div>
                  </div>
                </div>
                {diagnosticsCard.notices.length > 0 && (
                  <div className="mt-3 grid gap-2">
                    {diagnosticsCard.notices.map((notice) => (
                      <div
                        key={notice.text}
                        className={`rounded-xl border px-3 py-2 text-[11px] leading-relaxed ${
                          notice.tone === 'warning'
                            ...
                            : 'border-sky-200 bg-white/70 text-sky-800 dark:border-sky-400/20 dark:bg-white/[0.04] dark:text-sky-100'
                        }`}
                      >
                        {notice.text}
                      </div>
