...
function mapImageflowTaskAttempts(attempts: AgentImageflowTaskAttempt[]): AgentImageflowManagedAttempt[] {
  return attempts.map((attempt) => ({
    ...
    requestMode: attempt.request_mode,
    apiMode: attempt.api_mode,
    stream: attempt.stream,
    partialImageCount: attempt.partial_image_count,
...
export async function exportData(options: ExportOptions = { exportConfig: true, exportTasks: true }) {
  try {
    ...
    a.href = url
    a.download = `agent-imageflow-backup_${formatExportFileTime(new Date(exportedAt))}.zip`
    a.click()
