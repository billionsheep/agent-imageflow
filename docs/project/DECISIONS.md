...
- Impact: REST/CLI/Web 现在都可以在不翻深层 metadata 的前提下直接读 `final_delivery.counts/stories/scenes/final_assets`；caption 派生图会扁平暴露 `derived_from_asset_id` / `derivation_type`，`target_path` 采用 asset 优先、scene 兜底，响应继续不暴露 `local_path` 或任何 secret-like 字段。`P1-DLV-004/006/007` 的导出包、project delivery defaults 和治理联动继续作为后续独立切片，不在本轮落代码。

## 2026-06-27: NAS readable mirror 第一轮采用 batch-first materialize，不改事实源也不做后台自动同步

- Decision: `P1-DLV-005` 第一轮采用显式 materialize 的 batch-first readable mirror：默认 mirror root 为 `STORAGE_ROOT/final-delivery-mirror`，可通过 `FINAL_DELIVERY_MIRROR_ROOT` 覆盖；运维通过本地命令 `vag storage mirror-final` 或 Admin 受控 REST `POST /api/workspaces/{workspace_id}/projects/{project_id}/campaigns/{campaign_id}/final-delivery-mirror`，把 final/selected originals、thumbnails 和 `manifest.final.json` 写到 `workspaces/<workspace>/projects/<project>/campaigns/<campaign>/[sessions/<session>/]batches/<batch>`。mirror 优先复用 final asset 的 `target_path`，缺省回退到 `stories/<story_id>/<scene_id>`，所有写入路径都统一净化并拒绝 path traversal。
