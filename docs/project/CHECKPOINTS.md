...
- [x] P1 Project Production Context P1-PCTX-001 到 P1-PCTX-007 已完成：Project Visual Context 第一版复用 `project.metadata_json.visual_context`，支持角色卡、project reference binding、prompt recipe、CreateTask visual context 展开、REST/CLI/MCP 入口、任务/资产快照和自动化 examples。

## Remaining Product Gaps

- [x] MVP 产品闭环与生产 hardening 缺口已清零；当前仅剩本地开发环境清理类 follow-up，不属于产品缺口。
- [ ] Post-MVP Project Visual Context 后续项仍待执行：P1-PCTX-001 到 P1-PCTX-007 已完成服务端核心和自动化 examples；P1-PCTX-008 到 P1-PCTX-009 还需补最小 Web project context 面板和萌宠故事多 scene mock 回归；执行入口为 `issues/next-phase-p1-project-production-context.csv`。
