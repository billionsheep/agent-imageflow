...
同日已完成第一轮 `P1-DLV-001/002/003/008`：REST/CLI/Web 继续复用现有 `batch-manifest`，新增 `view=engineering|final_delivery` 双读模式；默认 `engineering` 保持兼容，`view=final_delivery` 额外提供面向人工复盘的 `final_delivery` block，按 `story/scene/final asset` 输出最终交付摘要、caption 派生扁平字段、continuity/visual context 和公开 delivery URLs。`P1-DLV-004/005/006/007` 的导出包、NAS readable mirror、project delivery defaults 与治理联动仍待后续单独推进。
...
| P1 专项 | `issues/next-phase-p1-final-delivery-nas-readable-export.csv` | 不改 canonical storage，只补 final delivery 语义、按 story/batch 导出和 NAS 可读 mirror，解决人工复盘、再次寻找和只读 NAS 浏览困难 | 第一轮 `P1-DLV-001/002/003/008` 已完成：`batch-manifest` 新增 `view=final_delivery` 与 Web/CLI 导出；`P1-DLV-004/005/006/007` 仍待后续 |
