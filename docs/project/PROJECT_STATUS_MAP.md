...
| P1 真实试用 follow-up | `issues/next-phase-p1-scope-management-usability-followup.csv` | 修复 Scope 管理真实试用中的高频摩擦：非空 scope 无法直接删除、确认弹窗层级点不到、页面无法滚动到底、输入框 `@` 参考图提示误导 | 已记录并确认级联删除语义；后续实现需 Admin 受控、二次确认、audit、storage 清理，且 MCP 不开放 destructive tools |
