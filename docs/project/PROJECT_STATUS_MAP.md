...
下一阶段产品计划已落地为独立入口：`issues/next-phase-p1-pet-account-real-workflow-trial.csv` 用于低并发真实萌宠账号试用；`issues/next-phase-p1-story-continuity-mvc.csv` 是外部评审后收敛出的第一执行切片，限定为 3 格、无字、顺序生成、人工选图、真实参考图参与；`issues/next-phase-p1-story-continuity-comic-workflow.csv` 作为上位路线保留；`issues/next-phase-p1-caption-edit-lineage.csv` 用于固定 asset 加字 edit 的派生关系，但 Web 加字入口和批量 caption 后置到 Story MVC 之后；`docs/project/STORY_CONTINUITY_AGENT_GUIDE.md` 用于指导额外 agent 负责连续叙事、分镜和重试策略。平台继续作为图片资产事实源，不扩成漫画编辑器或运营后台。
