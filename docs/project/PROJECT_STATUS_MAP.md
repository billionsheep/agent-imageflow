...
下一阶段产品计划已落地为独立入口：`issues/next-phase-p1-pet-account-real-workflow-trial.csv` 用于低并发真实萌宠账号试用；`issues/next-phase-p1-story-continuity-comic-workflow.csv` 用于 Story Bible、Panel Plan、reference roles 和 Story Review；`issues/next-phase-p1-caption-edit-lineage.csv` 用于固定 asset 加字 edit 的派生关系；`docs/project/STORY_CONTINUITY_AGENT_GUIDE.md` 用于指导额外 agent 负责连续叙事、分镜和重试策略。平台继续作为图片资产事实源，不扩成漫画编辑器或运营后台。
