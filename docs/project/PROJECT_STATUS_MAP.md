...
V1 文档入口已收敛：新会话和新 agent 默认从 `docs/project/README.md` 进入；CSV 状态从 `issues/README.md` 进入；历史实现 slice 从 `docs/project/stories/README.md` 进入。后续维护默认不要求通读全部历史 slice 或 CSV，已完成的 P0/P1/P2 CSV 不复开。
...
2026-06-27 新增的交付层判断是：当前真正痛点不是 agent 找不到结果，而是人工复盘、二次定位和 NAS 浏览困难。为避免牵动 cleanup / integrity / restore / audit，后续不直接改 canonical storage 的 `workspace/project/campaign/asset_id` 结构，而是新增 `issues/next-phase-p1-final-delivery-nas-readable-export.csv`，用 final delivery contract、story/batch export 和 NAS readable mirror 解决“按组找图”和“交付给人看”这件事。

同日已完成 `P1-DLV-001/002/003/005/008`：REST/CLI/Web 继续复用现有 `batch-manifest`，新增 `view=engineering|final_delivery` 双读模式；默认 `engineering` 保持兼容，`view=final_delivery` 额外提供面向人工复盘的 `final_delivery` block，按 `story/scene/final asset` 输出最终交付摘要、caption 派生扁平字段、continuity/visual context 和公开 delivery URLs。进一步新增 batch-first NAS readable mirror：运维可通过 `vag storage mirror-final` 或 Admin 受控 REST，把 final/selected originals、thumbnails 和 `manifest.final.json` materialize 到 `workspaces/<workspace>/projects/<project>/campaigns/<campaign>/[sessions/<session>/]batches/<batch>`，优先复用 `target_path`，缺省回退到 `stories/<story_id>/<scene_id>`，并统一拒绝 path traversal。剩余的是 `P1-DLV-004/006/007` 的导出包、project delivery defaults 与治理联动。

2026-06-26 收敛出的 `v0.2 MCP Production Hardening` 已在 2026-06-27 完成整轮闭环：`V02-MCPH-002` 到 `V02-MCPH-011` 均已关闭。范围覆盖 `AGENT_SETUP_TOKEN` / `X-Agent-Setup-Token` 非 destructive bootstrap、Project Visual Context `reference_diagnostics`、caption `speaker_character_id/bubble_anchor/tail_direction` 语义、`story_context_v1.panel_plan` 状态转场、caption derivative `delivery_role/final_delivery`、provider partial-success 口径、单资产 `asset_summary`、NAS/self-host 治理边界和本地 packaged review 路径。最新一轮本地真实 provider 回归已经证明：2 格情侣文案卡、3 格低背景连续故事和 caption derivative 在低并发下可用，final-delivery manifest、delivery URL、reference participation 和 continuity metadata 都可判定。NAS 适配继续保持 P1 边界：不阻断当前生成链路，不做内置 WebDAV/SMB server，也不让物理文件夹取代 DB/metadata 事实源。
  ...
  root((Agent ImageFlow))
    ...
    下一步
      服务器部署演练
        ...
        Backup Rollback
      真实业务生产试用
        MCP First Pet Story Workflow
        Story Continuity Agent
        Web Review And Manifest
...
| 萌宠小红书账号图片生产 | 从最小闭环转入 MCP-first 生产试用 | 建独立 project/campaign，由外部 Story Continuity Agent 产出 3-6 格 Story Bible / Panel Plan / `story_context_v1`，通过 MCP 顺序生成 panel task，平台沉淀 task/asset/metadata、上一格 selected 引用、Production View 审图状态和 selected manifest；Web 只做审图/管理/交付，不做主创作 | Usage Tracking、真实视觉质检、ZIP、多选下载、CLI/MCP regenerate command、Web 加字入口继续后置；真实 provider 执行前仍需费用确认 | 当前主线是图片资产生产、审看和交付，不做账号运营系统 |
...
| 后续 P1 | Pet Account Real Workflow Trial | 按 `docs/project/PET_STORY_PRODUCTION_WORKFLOW.md` 执行 MCP-first 3-6 格萌宠故事生产，观察外部 Story Continuity Agent、平台资产模型、Web 审图和 manifest/NAS 交付摩擦 | CSV 已从验收清单收敛为必须生产步骤 + 可选证据记录；下一步准备真实 project/campaign/context，费用确认后低并发执行 |
...
| P1 专项 | `issues/next-phase-p1-final-delivery-nas-readable-export.csv` | 不改 canonical storage，只补 final delivery 语义、按 story/batch 导出和 NAS 可读 mirror，解决人工复盘、再次寻找和只读 NAS 浏览困难 | `P1-DLV-001/002/003/005/008` 已完成：`batch-manifest` 新增 `view=final_delivery`，并可把 final/selected originals、thumbnails 和 `manifest.final.json` materialize 到 batch-first readable mirror；`P1-DLV-004/006/007` 仍待后续 |
...
| 后续 P2 | Usage Tracking、Caption UI / batch caption、Export Pack ZIP、Deployment Secret Hardening | 使用记录、Web 加字入口、批量 caption、小批量交付包、部署安全状态和 non-exposure regression | Caption lineage MCP-first contract 已完成；其余需要重新拆独立 CSV |
| 后续 P3 | 真实视觉质检 | 视觉一致性质量判断，先做人工辅助再评估 AI 自动质检 | 后置 |
| 暂不做 | 自媒体运营、自动发布、模板市场、通用 DAM、SaaS 注册计费 | 会把项目拖离资产生产平台 | 不做 |

## 推荐下一步

当前不要再次执行 P1 Asset Production Readiness CSV、P1 Web Performance / Startup CSV、P1 Provider Throughput & Reliability CSV、P1 Web Console Auth & Asset Visibility CSV、P1 Web UX Smoothness CSV、P1 Project Production Context CSV、P1 Batch Story Export Foundation CSV、P2 Web Operator Review Console CSV、P1 Deployment Release Pipeline CSV 或 P1 Web Console Auth Gate / Localization / Product Fit CSV。下一步建议继续 V1 之后的独立推进：第一优先仍是 `Server Deployment Rehearsal`，补 HTTPS/Caddy 正式同源入口、浏览器 Admin delivery smoke、restore 和改 tag 回滚。第二优先是 `Pet Account Real Workflow Trial`，按 `docs/project/PET_STORY_PRODUCTION_WORKFLOW.md` 让外部 Story Continuity Agent 通过 MCP 生产 3-6 格萌宠故事资产；mock 3 格数据链路和真实 provider canary 只作为可选证据，不要把每个 smoke 变成产品需求。第三优先是 `Caption Edit Lineage` 与 `Settings Information Architecture`。新 agent 默认从 `docs/project/README.md`、`issues/README.md`、`docs/project/PET_STORY_PRODUCTION_WORKFLOW.md` 和 `docs/project/STORY_CONTINUITY_AGENT_GUIDE.md` 开始，不再通过遍历全部历史文档来判断当前状态。
