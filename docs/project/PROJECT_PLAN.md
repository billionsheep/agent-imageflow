...
23. MCP Service Pack: In progress. `issues/next-phase-p1-mcp-service-pack.csv` 已作为独立 P1 小切片存在；当前已补 `docs/project/MCP_SERVICE_GUIDE.md`、`examples/mcp/agent-imageflow.local.json`、`examples/mcp/create-pet-scene.json` 和 `examples/mcp/smoke.md`，让新 agent 可直接照文档完成 tools/list、mock create task、get task、list assets 和 get delivery info。当前已完成 JSON parse 与静态检查，但人工 mock smoke 尚未回填 evidence，因此暂不标 done。该切片不做远程 HTTP MCP、不下发 provider key、不改 tool schema、不默认运行真实 provider。
24. Character Reference Intake and Consistency: Partially done / backend foundation done. 已新增 `issues/next-phase-p1-character-reference-intake-consistency.csv` 作为后续 P1 角色一致性补强切片；子代理 B 已完成后端 input-file promote 为 project reference asset、character primary/reference asset 字段与校验、CreateTask 自动携带角色参考图、task/asset 快照中的 provider reference participation diagnostics，以及参考图读取失败的用户可读错误语义。Web 角色卡缩略图、Web 可视化确认和完整端到端 smoke 仍未完成。该切片不做通用 DAM、AI 自动视觉质检或真实 provider benchmark。
25. Web Review Feedback and Stability: Planned. 已新增 `issues/next-phase-p1-web-review-feedback-stability.csv` 作为 P1 Web UX Smoothness 后的真实试用 follow-up；目标是让 Select/Reject、Production View scene actions 和下拉切换都有稳定、局部、可见的状态反馈，不再只靠 toast 或整页闪烁。
26. Safe Delete and Trial Reset: Planned. 已新增 `issues/next-phase-p1-safe-delete-and-trial-reset.csv` 作为后续 P1 数据生命周期切片；目标是让 Admin Web/REST/CLI 能安全归档、删除废图/失败任务并按 batch/session/campaign 重置试用数据。该切片必须复用 dry-run、二次确认、selected/published/approved 保护、audit 和备份说明；MCP 第一轮不开放 workspace/project/campaign 硬删除。
