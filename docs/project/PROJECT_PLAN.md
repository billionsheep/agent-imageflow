...
23. MCP Service Pack: Done. `issues/next-phase-p1-mcp-service-pack.csv` 已完成；当前已补 `docs/project/MCP_SERVICE_GUIDE.md`、`examples/mcp/agent-imageflow.local.json`、`examples/mcp/create-pet-scene.json` 和 `examples/mcp/smoke.md`，让新 agent 可直接照文档完成 tools/list、mock create task、get task、list assets 和 get delivery info；示例已修正为单行 JSON-RPC，并使用默认 seed scope，不依赖预配置角色卡。MCP mock smoke 已创建 `task_2c3706988499001b9715`，产出 `asset_6ba86dbe71f8c5ac4a57` 和 selected `asset_bf3a0db6a83f7542c1b7`，delivery info 返回 `download_url/thumbnail_url/metadata_url`。该切片不做远程 HTTP MCP、不下发 provider key、不改 tool schema、不默认运行真实 provider，也不提供 destructive tools。
...
26. Safe Delete and Trial Reset: Partial / asset archive/restore now connected. 已新增 `issues/next-phase-p1-safe-delete-and-trial-reset.csv` 作为 P1 数据生命周期切片；当前已扩展 `vag storage cleanup-preview/execute`，支持按 `asset_id/task_id/session_id/batch_id/story_id` 定位候选；新增 Admin-only REST `storage-cleanup-preview/execute`，继续要求 dry-run token 或明确确认，复用 selected/published/approved 保护和审计；Web `ScopeManagerModal` 现已新增当前 campaign 的数据清理面板，可调用 cleanup preview/execute、展示 candidate/file/bytes/by_reason/protected 摘要与截断 token，并要求输入确认短语后执行。单 asset `archive/restore` 已进入 Admin REST/Web/CLI，归档使用 `deprecated` 存储状态并对外显示 `archived`，恢复回 `generated`；默认 cleanup 不再清理 archived，只有显式 `--deprecated` / `include_deprecated=true` 才会物理清理归档资产。剩余是 task/input-file 级 reset、完整 browser smoke 和生产备份演练。MCP 第一轮不开放 workspace/project/campaign/asset destructive tools。
27. Server Deployment Rehearsal: Partial / Volcengine update smoke completed. 已新增 `issues/next-phase-p1-server-deployment-rehearsal.csv` 和 `docs/project/stories/slice-053-server-deployment-rehearsal.md`，把 GHCR pull、prod compose first boot、HTTPS 同源入口、Admin Web mock task、MCP stdio smoke、Postgres + storage restore、`IMAGE_TAG` 回滚和可选 1 图真实 provider canary 拆成独立验收项；`docs/project/SERVER_DEPLOYMENT_GUIDE.md` 已补部署演练证据模板。2026-06-23 已在 Volcengine `/opt/huoshan/apps/agent-imageflow` 对旧服务完成升级前备份、GHCR `main` pull/up、临时 HTTP health/Web smoke、MCP `tools/list` 和 mock benchmark smoke，未读取或打印 `.env.prod`、GHCR token、provider key、Admin cookie/session 或 project key；剩余是 HTTPS/Caddy 正式入口、浏览器 Admin/delivery smoke、restore 和 `IMAGE_TAG` 回滚。
28. Scope Management Usability Follow-up: Done. `issues/next-phase-p1-scope-management-usability-followup.csv` 中 P1-SCOPE-UX-001 到 P1-SCOPE-UX-004 已完成：非空 workspace/project/campaign 删除已从 empty-only 调整为 Admin 受控级联删除，明确删除子级 project/campaign/task/asset/version/review/delivery 记录，并继续通过 Service 层清理 storage scope 目录；确认弹窗层级提升，Scope 管理 modal 可在小视口滚动，InputBar 不再默认宣传未接入 Project Reference 的 `@` 功能。MCP 仍不开放 workspace/project/campaign/asset destructive tools。验证已覆盖容器化 Go tests、Web tests/build、compose config、diff check、MCP mock smoke 和 browser campaign cascade delete smoke；browser smoke 使用 `cmp_smoke_delete_1782353313` / `asset_c214e3745f97c30afd6b` 验证 Web “级联删除”后 campaign 消失、asset metadata/original 404、MCP list 返回空。
29. Runtime/Auth/Accessibility/Lifecycle Closure: Mostly done / smoke pending. 已新增 `issues/next-phase-p1-runtime-auth-accessibility-lifecycle-closure.csv`，完成 Admin runtime status 的安全 build/provider/auth 摘要、Docker/GitHub Actions build metadata 注入、Web API/Web 版本摘要与 mismatch 提示、登录失败文案收敛、Scope selector 中文 `aria-label`、单 asset archive/restore 以及 archived 默认 cleanup 保护。剩余是部署新镜像后的浏览器 smoke、mock 角色一致性 smoke，以及人工确认费用后的 1 图真实 reference canary；本轮不运行真实 provider，不读取或打印任何 key/secret/cookie/session。
