...
28. Scope Management Usability Follow-up: Planned. 已新增 `issues/next-phase-p1-scope-management-usability-followup.csv` 记录真实 Web 试用中的四个问题：非空 workspace/project/campaign 删除被 empty-only 规则阻断、Scope 删除确认弹窗层级点不到、Scope 管理内容无法滚动到底、输入框 `@` 参考图提示与 server-first Project Reference 语义不一致。用户已确认 scope 删除应支持级联删除并包含 selected/approved/published 资产；后续实现必须保持 Admin 受控、二次确认、audit、storage 清理和 MCP 不开放 destructive tools 的边界。
