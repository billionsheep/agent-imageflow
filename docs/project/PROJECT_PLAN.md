...
23. MCP Service Pack: Planned. 已新增 `issues/next-phase-p1-mcp-service-pack.csv` 作为后续 P1 小切片；目标是让新 agent 只看一份 MCP service guide 和配置/调用示例，就能完成 tools/list、mock create task、get task 和 get delivery info。该切片不做远程 HTTP MCP、不下发 provider key、不改 tool schema、不默认运行真实 provider。
24. Character Reference Intake and Consistency: Planned. 已新增 `issues/next-phase-p1-character-reference-intake-consistency.csv` 作为后续 P1 角色一致性补强切片；目标是把用户上传/裁切的角色图从 input-files 提升为 project reference asset，绑定到 character primary/reference assets，并让 MCP/Web 任务可明确携带参考图、记录 provider reference participation 和在 Web 上可视化确认。该切片不做通用 DAM、AI 自动视觉质检或真实 provider benchmark。
