...
- One-sentence positioning: 面向 AI agent、自动化系统和内容工作流的图片资产生成、落盘、选优、复用与交付平台。
- Target user: 使用 Codex / Claude / Cursor / 自动化脚本生产内容或业务素材的独立开发者、技术创作者、内容团队和内部工具开发者。
- Core pain point: 现有生图工具大多面向网页人工操作，AI 或自动化流程即使能生成图片，也经常缺少稳定文件路径、URL、asset_id、版本、元数据、质量策略、选优状态和可复用交付接口。
- Desired outcome: 外部 AI 或系统能通过结构化接口创建图片资产，并拿到可追踪、可选优、可复用、可交付的正式结果。
...
Codex 或 Claude 在生成文章、README、小说章节、运营文案时，通过 MCP 创建图片任务，平台返回候选图、文件路径、URL 和元数据。调用方可以直接使用结果，也可以从候选图中标记 selected / rejected 后再用于正文、发布系统或素材库。

### 小说与长内容配图

用户为章节、人物、场景、物品或封面创建批量图片任务。平台保留角色设定、风格、参考图、生成版本和选优记录，避免每次从零手工整理。
...
- 支持轻量资产状态：generated、selected、rejected、published、deprecated。第一版不要求每张图片走人工审核闸门。
- 提供 REST API 或 CLI 的最小入口。
- 提供 MCP tool 草案，让 Codex / Claude 后续可调用。

### Should Have If Cheap

- 生成多张候选图并通过人工或自动策略选出推荐图。
- 缩略图预览。
- JSON metadata 导出。
...
- 不负责正文事实编写。
- 不把“人工审核”作为单体/小团队默认必经流程；需要强审核的团队可后续通过策略配置启用。
...
3. 用户或 AI 在 campaign 下创建图片任务，说明标题、主题、尺寸、风格、输出数量、prompt 模板和质量偏好。
4. 平台把任务入队，并调用配置好的生图后端。
5. Worker 下载或接收生成结果，保存原图、生成缩略图，并登记资产和版本元数据。
6. 用户、agent 或选优逻辑在候选图中标记 selected、reject 或 regenerate；小团队场景下默认不强制逐张审核。
7. 平台返回稳定 URL、本地路径、缩略图、metadata 和可复用交付信息；selected 只是推荐/首选标记，不是默认交付闸门。
...
- Selection workflow capability: 支持候选图选优、推荐图标记、reject/regenerate，而不是强制每张图人工审核。
...
- Web UI input: 给人工创建、预览、选优和管理任务使用。
...
- Given 多张候选图，when 用户或系统选中其中一张，then 资产状态变为 selected，并可返回稳定路径或 URL。
...
- Given 一个内容账号 campaign，when 批量创建封面图任务，then 每个任务都能生成候选图、缩略图和可选优资产。
...
- 第一版默认面向个人或小团队，不把人工审核作为强制流程；质量优先通过 prompt 模板、风格预设、参考图、生成参数和自动选优策略提升。
- 先接一个 provider 足够验证产品价值。
- 本地优先或轻量自托管会比一开始做云 SaaS 更适合验证。

## Open Questions

- selected 资产后续是否需要直接推送到某个目标系统，例如 Notion、GitHub repo、CMS？
...
- 第一阶段资产生成后默认可预览和交付；`selected` 用于标记推荐结果，`rejected` 用于排除候选，不默认阻断交付。
